...
xv6-public을 기반으로 시스템 콜, nice 기반 우선순위 스케줄러와 lazy page allocation을
구현한 운영체제 과제·기술 사례입니다. 2023년 학부 구현을 기반으로 2026년에 비선점
조건과 독립 테스트를 다시 점검했습니다.

## 요약

| 해결 대상 | 구현한 방법 | 핵심 코드 | 결과 |
|---|---|---|---|
...
> nice 선택 정책과 별개로 남아 있던 타이머 기반 자동 `yield()`를 제거해 과제의
> 비선점 조건을 반영하고, 명시적 양보 경로는 유지했습니다.

## 자료 구성

- 시스템 콜·스케줄러·지연 할당 구현을 포함한 xv6 source
...
정책 점검 과정에서 기존 `trap.c`가 타이머 인터럽트에서도 `yield()`를 호출하여
...
과제의 핵심 요구인 지연 할당 경로는 구현되어 있습니다. 메모리 부족, 유효하지 않은 주소,
...
과제에서 제공된 테스트는 저작권과 기여도 구분을 위해 포함하지 않았습니다. 저장소에는
각 구현의 실행 조건을 확인하기 위해 별도로 작성한 테스트만 포함합니다.
